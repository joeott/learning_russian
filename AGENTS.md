...
## Golden rules
1. **`content/content.json` is the built content contract.** Never hand-edit
   the generated files (`web/content.js`, `anki/*.txt`, `printable/cheatsheet.html`,
   `web/audio.js`). Edit `scripts/build_content.py` (the phrase data) and
   `courses/russian_family_visit/course.yaml` (course metadata), then rebuild.
...
5. **Run validation tests after engine/app changes:** `tools/zastolom test`.
...
## Layout
```
content/        content.json  (built content contract, 109 items)
...
prompts/        iterative local improvement-loop prompt
tools/          zastolom CLI
.codex/        repo-local agent definitions
...
- Improve the web app repeatedly → `prompts/improve-loop.md` or the **loop-runner** agent.

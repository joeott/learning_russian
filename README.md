...
The web app now tracks stage-specific mastery, delayed recall, and due reviews locally: recognition, recall, cloze, dictation, stress, pronunciation, back-translation, contrast, listening, production, and role-play are scheduled separately, with lapse/error data feeding repair drills. Listening practice includes a fading-caption ladder and speed controls so assisted comprehension is scheduled as a harder review. Role-play self-rating is criterion-aware, so missed scenario goals become targeted repair signals.

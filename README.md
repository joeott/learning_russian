...
| 3 | **Anki deck** | Spaced repetition on your phone — a 420-card base deck plus optional contextual cloze/dictation/stress/pronunciation/scenario siblings. | [`anki/`](anki/) — see its README to import |
...
source/         Ekaterina's original guide + the verified research it was built from
              + candidate_materials.md for curated input candidates
...
The web app now tracks stage-specific mastery, delayed recall, response latency, repair-focus history, due reviews, and daily readiness history locally: recognition, recall, cloze, dictation, stress, pronunciation, back-translation, contrast, listening, production, and role-play are scheduled separately, with lapse/error data feeding repair drills. Wrong answers now show a brief targeted repair focus from the canonical error taxonomy before asking the learner to classify the miss, and the Home dashboard summarizes top repair-focus patterns. Listening practice includes a fading-caption ladder, speed controls, and a room-noise pass so assisted comprehension is scheduled as a harder review while real-world listening remains unassisted. Role-play self-rating is criterion-aware, and the Home dashboard turns cumulative missed scenario goals into targeted repair signals. The `tools/zastolom flow` check now verifies lesson-locking, Home, Learn, every Drill stage, and analytics state in a real browser.
